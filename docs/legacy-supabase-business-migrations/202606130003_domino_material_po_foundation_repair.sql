...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'purchase_orders_shipment_risk_check'
...
drop policy if exists material_requirements_delete_procurement_roles on public.material_requirements;
...
drop policy if exists material_pos_delete_procurement_drafts on public.material_purchase_orders;
...
drop policy if exists material_po_items_delete_procurement_drafts on public.material_po_items;
...
create policy material_requirements_delete_procurement_roles on public.material_requirements
for delete using (
  public.has_app_role('Owner'::public.app_role, 'Planning'::public.app_role, 'Purchasing'::public.app_role)
);
...
create policy material_pos_delete_procurement_drafts on public.material_purchase_orders
for delete using (
  status = 'Draft' and
  public.has_app_role('Owner'::public.app_role, 'Planning'::public.app_role, 'Purchasing'::public.app_role)
);
...
create policy material_po_items_delete_procurement_drafts on public.material_po_items
for delete using (
  ...
  exists (
    ...
    where mpo.id = public.material_po_items.material_po_id
